...
We found the subject of the paper very interesting, moreover the paper is nicley written. We appreciate the fact that the authors made the code available on github. This allowed us to thorougly analize all the steps of learning the model.   

## Part 1: The Teacher's Questions

### 1. Is the code consistent with the paper?
Yes, it is. The model's parameters are initialized by some default method from pythorch. 
...
We focused on word translation retrieval using CSLS KNN. First we reprodeced the results for EN to ES task, then we learned the model for EN to PL translation task. 

### EN to ES

To start with, we ran 5 epochs of the project (unsupervised version) for EN to ES embeddings mapping.
...
Clearly, the model works as stated in the paper.


Then, we ran EN-ES for huge amount of epochs but it automatically stoped near epoch 25, when the improvement was too small.
...
Again, we ran 5 epochs of the project for EN to PL embeddings mapping.
...
Clearly, after 5 epochs for the EN-PL task the model works terribly badly. But for EN-ES it worked OK.
...
This time, we run EN-PL tasks for huge amount of epochs but it automatically stoped near epoch 25, when the improvement was too small.
...
To conclude, the paper is nicely written, the subject is interesting, the method gives good performance.
